...
export default function SalesLeads() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="visible" className="space-y-5">
              ...
              <motion.div key={l.id} layout variants={cardAnim} initial="hidden" animate="visible" exit={{ opacity: 0, scale: 0.95 }}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border/50">
                  ...
                  <div className="relative">
                    <select className="text-[11px] bg-background border border-border rounded-lg px-2.5 py-1.5 pr-7 text-foreground outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/10 transition-all appearance-none cursor-pointer font-medium"
                    ...
                    <svg className="absolute right-2 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground/50 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <ModalForm title="Add New Lead" open={modalOpen} onClose={() => setModalOpen(false)}>
        <form onSubmit={handleSave} className="space-y-5">
          {[
            { label: "Full Name", key: "name" as const, placeholder: "e.g. Sarah Chen", icon: Users },
            { label: "Contact Info", key: "contact" as const, placeholder: "Email or phone number", icon: Mail },
            { label: "Vehicle Interested", key: "vehicleInterested" as const, placeholder: "e.g. 2024 Toyota Camry XSE", icon: Car },
          ].map((f) => (
            <div key={f.key}>
              <label className="block text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">{f.label}</label>
              <div className="relative">
                <f.icon className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/50" />
                <input
                  className="w-full h-11 rounded-xl border border-border bg-background pl-10 pr-3 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all placeholder:text-muted-foreground/50"
          ...
          <div>
            <label className="block text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Lead Source</label>
            <div className="relative">
              <Globe className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/50 pointer-events-none z-10" />
              <select
                className="w-full h-11 rounded-xl border border-border bg-background pl-10 pr-8 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all appearance-none cursor-pointer"
                value={form.source}
          ...
          <div>
            <label className="block text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Initial Status</label>
                ...
                return (
                  <button
                    ...
                    className={`h-9 rounded-lg text-xs font-semibold transition-all border ${
                      form.status === s
                        ? `${sc.bg} border-current shadow-sm`
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            type="submit"
            className="w-full h-11 rounded-xl bg-primary text-primary-foreground text-sm font-semibold shadow-md mt-1 flex items-center justify-center gap-2"
            style={{ boxShadow: "0 4px 14px hsl(var(--primary) / 0.3)" }}
          >
            <Plus className="h-4 w-4" /> Add Lead
          </motion.button>
